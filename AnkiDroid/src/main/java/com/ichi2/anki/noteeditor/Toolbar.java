...
//        setClick(R.id.note_editor_toolbar_button_cover, "<span class=\"ak-cover\">", "</span>");
        ...
        findViewById(R.id.note_editor_toolbar_button_font_color).setOnClickListener(l -> displayFontColorDialog());
    ...
    private void displayFontColorDialog() {
        String[] results = getResources().getStringArray(R.array.html_color_codes);

        // Might be better to add this as a fragment - let's see.
        new MaterialDialog.Builder(getContext())
                .items(R.array.html_color_code_labels)
//                .itemsColorRes(R.color.review_button_text_color)
                .itemsCallback((dialog, view, pos, string) -> {
                    String prefix = "<span style=\"color:" + results[pos] + "\">";
                    String suffix = "</span>";
                    TextWrapper formatter = new TextWrapper(prefix, suffix);
                    onFormat(formatter);
                })
                .title(R.string.menu_font_color)
                .show();
    }
